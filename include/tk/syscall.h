...
typedef struct t_devreq {
	...
	VP	buf;		/* I:Input/output buffer address */
...
IMPORT ID tk_opn_dev( UB *devnm, UINT omode );
IMPORT ER tk_cls_dev( ID dd, UINT option );
IMPORT ID tk_rea_dev( ID dd, W start, VP buf, W size, TMO tmout );
IMPORT ER tk_srea_dev( ID dd, W start, VP buf, W size, W *asize );
IMPORT ID tk_wri_dev( ID dd, W start, VP buf, W size, TMO tmout );
IMPORT ER tk_swri_dev( ID dd, W start, VP buf, W size, W *asize );
...
IMPORT ID tk_ref_dev( UB *devnm, T_RDEV *pk_rdev );
IMPORT ID tk_oref_dev( ID dd, T_RDEV *pk_rdev );
IMPORT INT tk_lst_dev( T_LDEV *pk_ldev, INT start, INT ndev );
IMPORT INT tk_evt_dev( ID devid, INT evttyp, VP evtinf );
IMPORT ID tk_def_dev( UB *devnm, T_DDEV *pk_ddev, T_IDEV *pk_idev );
