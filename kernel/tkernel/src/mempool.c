 ...
 */
EXPORT VP knl_get_blk( MPLCB *mplcb, W blksz )
{
	QUEUE	*q, *aq, *aq2;
	IMACB*	imacb = (IMACB*)&(mplcb->mplsz);
 ...
 * Free memory block 
 */
EXPORT ER knl_rel_blk( MPLCB *mplcb, VP blk )
{
	QUEUE	*aq;
	IMACB*	imacb = (IMACB*)&(mplcb->mplsz);
 ...
 */
LOCAL WSPEC knl_wspec_mpl_tfifo = { TTW_MPL, NULL,        mpl_rel_wai };
LOCAL WSPEC knl_wspec_mpl_tpri  = { TTW_MPL, mpl_chg_pri, mpl_rel_wai };

/*
 * Get variable size memory block 
 */
SYSCALL ER tk_get_mpl_impl( ID mplid, W blksz, VP* p_blk, TMO tmout )
{
	MPLCB	*mplcb;
	VP	blk = NULL;
	ER	ercd = E_OK;
